module Process(update, viewTr, Model, Pid, decode, filter, thead, Action) where
...
viewTr : Signal.Address Pid -> Model -> Bool -> Html
viewTr address model selected =
   tbody_
      [ tr ((if selected then [highlight] else []) ++ [onClick address model.pid])
        [ td_ [text model.pid]
...
showCode v =
  String.join "" [v.mod, ":", v.fun, "/", toString v.args]
thead =
